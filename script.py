...
def gui_mode():
    # Create a Tcl interpreter without creating a Tk instance
    root = tk.Tk()
    root.withdraw()  # Hide the main window

    # This will avoid the 'no display name and no $DISPLAY environment variable' error
    try:
        root.tk.call('info', 'hostname')
    except tk.TclError:
        # If TclError occurs, just exit the function
        return
